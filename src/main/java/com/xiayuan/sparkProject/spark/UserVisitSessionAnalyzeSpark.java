...
//        .setMaster("local")
//        .set("spark.default.parallelism", "4")  调节并行度
//        .set("spark.serializer", "org.apache.spark.serializer.KryoSerializer");  Kryo序列化机制
//        .registerKryoClasses(new Class[]{CategorySortKey.class})  注册你自定义的类
...
//        HiveContext hc = new HiveContext(sc);
        ...
        //首先 查询出来指定的任务，并获取任务的查询参数
        long taskid = ParamUtils.getTaskIdFromArgs(args,Constants.SPARK_LOCAL_TASKID_SESSION);
        Task task = taskDAO.findById(taskid);
        JSONObject taskParam = JSONObject.parseObject(task.getTaskParam());
        System.out.println("taskid是"+taskid);
        System.out.println("task参数是"+taskParam.toString());

        //如果要进行session粒度的数据聚合
        //首先要从user_visit_action表中，查询出来指定日期范围内的数据
        JavaRDD<Row> actionRDD = getActionRDDByDateRange(sqlContext, taskParam);

        //获得<session_id, actionRDDInfo>类型的数据
        // user_visit_action中的数据
        JavaPairRDD<String,Row> session2actionRDD = getSessionid2ActionRDD(actionRDD);
//        System.out.println("session2actionRDD的take10"+session2actionRDD.take(10));
...
//        session2actionRDD.checkpoint();
        ...
        /**
         * 首先，将行为数据按照session_id进行groupByKey分组
         * 此时的数据粒度就是session粒度，然后可以将session粒度的数据与用户信息数据进行jion
         * 然后就可以获取到session粒度的数据，同时数据里面包括了session对应的user信息
         *
         */
        JavaPairRDD<String, String> sessionid2AggrInfoRDD = aggregateBySession(sqlContext, session2actionRDD);

        //sessionid2AggrInfoRDD:null,sessionid=912dd1945f2741a79dfcc099e84857a9|searchKeywords=蛋糕,日本料理|clickCategoryIds=0,81,39|visitLength=1683|stepLength=6|age=16|professional=professional14|city=city20|sex=female
//        System.out.println("sessionid2AggrInfoRDD的take10："+sessionid2AggrInfoRDD.take(10));

        /**
         * 接着，针对session粒度的聚合数据，按照使用者指定的筛选参数进行数据过滤
         * 相当于我们自己编写算子，要是访问外面的任务参数对象的匿名内部类（算子函数），
         * 访问外部对象，是要给外部对象使用final修饰的
         */
...
//        Accumulator<String> sessionAggrStatAccumulator = sc.accumulator("", new SessionAggrStatAccumulator());
         ...
         */
        Accumulator<String> sessionAggrStatAccumulator1 = sc.accumulator("", new SessionAggrStatAccumulator());

        JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD = filterSessionAndAggrStat(sessionid2AggrInfoRDD, taskParam, sessionAggrStatAccumulator1);

        filteredSessionid2AggrInfoRDD = filteredSessionid2AggrInfoRDD.persist(StorageLevel.MEMORY_ONLY());

        //filteredSessionid2AggrInfoRDD:null,sessionid=912dd1945f2741a79dfcc099e84857a9|searchKeywords=蛋糕,日本料理|clickCategoryIds=0,81,39|visitLength=1683|stepLength=6|age=16|professional=professional14|city=city20|sex=female
//        System.out.println("filteredSessionid2AggrInfoRDD的value是： " + filteredSessionid2AggrInfoRDD.count());

        //获取过滤后的user_visit_action中的数据
        ...
        //注意，要取消注释
        randomExtractSession(sc, taskid, filteredSessionid2AggrInfoRDD, session2actionRDD);
        calculateAndPersistAggrStat(sessionAggrStatAccumulator1.value(), task.getTaskid());
    ...
    private static JavaPairRDD<String, Row> getSessionid2detailRDD(JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD, JavaPairRDD<String, Row> session2actionRDD) {

        JavaPairRDD<String, Row> sessionid2detailRDD = filteredSessionid2AggrInfoRDD.join(session2actionRDD).mapToPair(
    ...
    /**
     * 获取SQLcontext
     * 如果在本地测试环境，那么就生成SQLContext对象
     * 如果在生产环境运行，那么就生成HiveContext
     *
     * @param sc
     * @return
     */
...
//                 * PairFunction
//                 * 第一个参数相当于是函数的输入，
//                 * 第二个参数和第三个参数相当于是函数的输出（Tuple），分别的tuple的第一个个第二个值
//                 */
...
//        );
        ...
        //到此为止，获取到的数据格式如下:<userid,partAggrInfo(sessionid,searchKeywords,clickCategoryIds)>
...
//                            if (clickCategoryId != Long.MAX_VALUE) {
                            ...
                            //计算session访问步长，一个session点击过几个页面
                            stepLength++;
                        ...
                        //但是，这一步聚合后，其实还需要将每一行数据，跟对应的用户信息进行聚合
        ...
        //将session粒度聚合数据，将相同key的value聚合在一起，搜索信息和点击信息与用户信息进行join
    ...
    private static JavaPairRDD<String, String> filterSessionAndAggrStat(
        ...
        /*
        * 为了使用后面的ValieUtils，所以首先将所有的筛选参数拼接成一个连接串
        *
        * _parameter是 将taskParam中的筛选条件拼接成一个连接串，并用这些
        * 筛选条件来筛选每个session
        *
        * */
        ...
        System.out.println("parameter是：" + parameter);
        ...
        JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD = sessionid2AggrInfoRDD.filter(
                ...
                new Function<Tuple2<String, String>, Boolean>() {
                    ...
                    private void calculateVisitLength(long visitLength) {
                        ...
                        } else if (visitLength > 600 && visitLength <= 180) {
    ...
    private static void calculateAndPersistAggrStat(String value, long taskid) {
        ...
        long session_count = Long.valueOf(StringUtils.getFieldFromConcatString(value, "\\|", Constants.SESSION_COUNT)) / 2;
    ...
    /**
     * 随机抽取session
     * 需求：每一次执行用户访问session分析模块，要抽取出100个session
     *
     * @param sessionid2AggrInfoRDD
    ...
    private static void randomExtractSession(
            ...
            final long taskid,
            JavaPairRDD<String, String> sessionid2AggrInfoRDD,
            final JavaPairRDD<String, Row> session2actionRDD) {
        //第一步，计算出每天每小时的session数量，获取<yyyy-MM-dd_HH,sessionid>格式的RDD
        //time2sessionidRDD格式<dateHour, aggrInfo>
...
//        Map<String, Long> longMap = time2sessionidRDD.countByKey();

        //第二步，使用按时间比例随机抽取算法，计算出每天每小时需要抽取的session的索引
...
//            dateHourCountMap.put(date, hourCountMap);
        ...
        //开始实现按时间比例随机抽取算法
        //总共抽取100个session，先按照天数，进行平分
        int extractNumberPerDay = 100 / dateHourCountMap.size();
        
        //dateHourCountMap格式  <date,<hour,(1,3,4,4789)>>
//        Map<String, Map<String, List<Integer>>> dateHourExtractMap = new HashMap<String, Map<String, List<Integer>>>();
        ...
        for (Map.Entry<String, Map<String, List<Integer>>> dateHourExtractEntry : dateHourExtractMap.entrySet()) {
            ...
            for (Map.Entry<String, List<Integer>> hourExtractEntry : hourExtractMap.entrySet()) {
                ...
                IntList fastutilExtractList = new IntList();
        ...
        JavaPairRDD<String, Tuple2<String, Row>> extractSessionDetailRDD = extractSessionidsRDD.join(session2actionRDD);
     ...
     * @return List<Tuple2<CategorySortKey, String>> top10CategoryList
     */
    private static List<Tuple2<CategorySortKey, String>> getTop10Category(
        ...
        //获取session访问过的所有品类id，访问过是指：点击过、下单过、支付过
        ...
        /**
         * 必须进行去重
         * 如果不去重，会出现重复的categoryid，排序会对重复的Categoryid的categoryInfo进行排序
         * 最后很可能拿到重复的数据
         */

        categoryidRDD = categoryidRDD.distinct();
//        System.out.println("categoryidRDD的take10："+categoryidRDD.take(10).size());

        /**
         * 第二步，计算各个品类的点击、下单和支付的次数
         *
         */
        ...
        // 计算各个品类的点击次数 <clickCategoryid,次数>
        JavaPairRDD<Long, Long> clickCategoryId2CountRDD = getClickCategoryId2CountRDD(sessionid2detailRDD);
//        System.out.println("clickCategoryId2CountRDD的take10："+clickCategoryId2CountRDD.take(10).size());

        //计算各个品类下单次数 <OrderCategoryId,次数>
         ...
         */
        //返回一个<categoryid, categoryId=?|clickCount=?|orderCount=?|payCount=?>
...
//        System.out.println("categoryid2countRDD的take10："+categoryid2countRDD.take(10).size());

        JavaPairRDD<CategorySortKey, String> sortedCategoryCountRDD = sortKey2countRDD.sortByKey(false);
//        System.out.println("sortKey2countRDD的take10："+sortKey2countRDD.take(10).size());
//        sortedCategoryCountRDD.foreach(
//                new VoidFunction<Tuple2<CategorySortKey, String>>() {
//                    @Override
//                    public void call(Tuple2<CategorySortKey, String> tuple) throws Exception {
...
//        );
        ...
        System.out.println(top10CategoryList.size());
    ...
    private static JavaPairRDD<Long, Long> getClickCategoryId2CountRDD(JavaPairRDD<String,Row> sessionid2detailRDD) {
        ...
        JavaPairRDD<Long, Long> clickCategotyRDD = clickActionRDD.mapToPair(
        ...
        JavaPairRDD<Long, Long> clickCategoryId2CountRDD = clickCategotyRDD.reduceByKey(
    ...
    private static JavaPairRDD<Long, Long> getOrderCategoryId2CountRDD(JavaPairRDD<String,Row> sessionid2detailRDD) {
        final JavaPairRDD<String, Row> orderAction = sessionid2detailRDD.filter(
                new Function<Tuple2<String, Row>, Boolean>() {
                    private static final long serialVersionUID = 1L;
        ...
        JavaPairRDD<Long, Long> orderCategoryIdRDD = orderAction.flatMapToPair(
                new PairFlatMapFunction<Tuple2<String, Row>, Long, Long>() {
                    private static final long serialVersionUID = 1L;
    ...
    private static JavaPairRDD<Long, Long> getPayCategoryId2CountRDD(JavaPairRDD<String,Row> sessionid2detailRDD) {
        ...
        JavaPairRDD<Long, Long> payCategoryIdRDD = payAction.flatMapToPair(
                new PairFlatMapFunction<Tuple2<String, Row>, Long, Long>() {
                    private static final long serialVersionUID = 1L;
                    ...
                    public Iterator<Tuple2<Long, Long>> call(Tuple2<String, Row> tuple) throws Exception {
                        Row row = tuple._2;
                        String payCategotyIds = row.getString(10);
                        String[] payCategoryIdsSplited = payCategotyIds.split(",");
...
//        JavaPairRDD<Long, Tuple2<Long, Optional<Long>>> tmpJoinRDD = categoryidRDD.leftOuterJoin(clickCategoryId2CountRDD);
    ...
    private static void getTop10Session(
        ...
        //获取到top10热门品类，被各个session点击的次数 <top10Categoryid, Session+Count>
         ...
         */
        JavaPairRDD<Long, Iterable<String>> top10Categoryid2SessionCountsRDD = top10Categoryid2SessionCountRDD.groupByKey();

        JavaPairRDD<String, String> top10SessionRDD = top10Categoryid2SessionCountsRDD.flatMapToPair(
                    ...
                    public Iterator<Tuple2<String, String>> call(Tuple2<Long, Iterable<String>> tuple) throws Exception {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            for (int i = 0; i < top10Sessions.length; i++) {
                                //如果当前i为null，那么直接将i位数据赋值为当前的sessionCount
                        ...
                        for (String sesisonCount : top10Sessions) {
                            if (sesisonCount != null){
                                String sessionid = sesisonCount.split(",")[0];
                                long count = Long.valueOf(sesisonCount.split(",")[1]);
         ...
         */
        JavaPairRDD<String, Tuple2<String, Row>> sessionidDetailRDD = top10SessionRDD.join(sessionid2detailRDD);

        sessionidDetailRDD.foreach(
