...
/**
 * 校验工具类
 * 校验工具类主要是校验数据中的指定字段是否在制定范围内以及是否有参数字段的值相同等功能。
 *
 * @author yeunsher
...
public class ValidUtils {
    ...
    public static boolean equal(String data, String dataField,
                                String parameter, String paramField){
        String paraFieldValue = StringUtils.getFieldFromConcatString(parameter, "\\|", paramField);
        if (paraFieldValue == null){
            return true;
        }

        String dataFieldValue = StringUtils.getFieldFromConcatString(data, "\\|", dataField);
        if (dataFieldValue != null){
            if (dataFieldValue.equals(paraFieldValue)){
                return true;
            }
        }
        return false;
